...
def nn_data(resolution: tuple, downsample: int, channel: int) -> tuple:
    ...
    subgrid_flux = np.transpose(sim_data.calc_subgrid_flux(), axes=(1, 0, 2, 3))[channel]
    ...
    output_tensor = torch.from_numpy(subgrid_flux).unsqueeze(1).float()
...
def snapshot_pred(rho: np.ndarray, temp: np.ndarray, pressure: np.ndarray, ux: np.ndarray, uy: np.ndarray, eint: np.ndarray, ps: np.ndarray, downsample: int, resolution: np.ndarray) -> np.ndarray:
    ...
    input_mean = np.load(f"/data3/home/dipayandatta/Subgrid_CGM_Models/conv_nn/flux_model_saves/cnn_{sim_data.resolution}_{downsample}_0_input_mean.npy")
    input_std = np.load(f"/data3/home/dipayandatta/Subgrid_CGM_Models/conv_nn/flux_model_saves/cnn_{sim_data.resolution}_{downsample}_0_input_std.npy")
    ...
    for channel in channels:
        ...
        model_path = f'/data3/home/dipayandatta/Subgrid_CGM_Models/conv_nn/flux_model_saves/cnn_{sim_data.resolution}_{downsample}_{channel}.pth'
        ...
        with torch.no_grad():
            output_mean = torch.from_numpy(np.load(f"/data3/home/dipayandatta/Subgrid_CGM_Models/conv_nn/flux_model_saves/cnn_{sim_data.resolution}_{downsample}_{channel}_output_mean.npy"))
            output_std = torch.from_numpy(np.load(f"/data3/home/dipayandatta/Subgrid_CGM_Models/conv_nn/flux_model_saves/cnn_{sim_data.resolution}_{downsample}_{channel}_output_std.npy"))
...
if __name__ == "__main__":
    ...
    for channel in channels:
        ...
        np.save(f"flux_model_saves/cnn_{resolution}_{downsample}_{channel}_input_mean.npy", input_mean.cpu().numpy())
        np.save(f"flux_model_saves/cnn_{resolution}_{downsample}_{channel}_input_std.npy", input_std.cpu().numpy())
        ...
        if output_std == 0:
            output_std = 1.0
        np.save(f"flux_model_saves/cnn_{resolution}_{downsample}_{channel}_output_mean.npy", output_mean.cpu().numpy())
        np.save(f"flux_model_saves/cnn_{resolution}_{downsample}_{channel}_output_std.npy", output_std.cpu().numpy())
        ...
        # Save the model
        torch.save(cnn_model.state_dict(), f'flux_model_saves/cnn_{resolution}_{downsample}_{channel}.pth')
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig(f"flux_loss_plots/cnn_{resolution}_{downsample}_{channel}_loss.jpg", dpi=500)
        plt.close()
